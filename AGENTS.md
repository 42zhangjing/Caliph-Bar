...
- 经典外观以 `Resources/Shapes/caliph-edge-tab.svg` 为母版（`SideNotchShape.swift`，等比 `210:1138`）；剪影外观以 `Resources/Shapes/caliph-edge-silhouette.svg` 为母版（`CanonicalSilhouette.swift`，等比 `600:1864`），均使用精确转录的矢量贝塞尔路径，不得近似重画、独立拉伸宽高或用通用圆角形状替换。
- 左右停靠必须严格镜像。
