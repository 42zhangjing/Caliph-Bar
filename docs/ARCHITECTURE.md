...
从 Anthropic 读取真实 usage。后台 Keychain 读取保持非交互式。凭据或接口读取失败时不返回估算快照，并保留具体失败原因。近期有效快照可标记为 `STALE`；没有可用快照时为 `OFFLINE`。本地日志的费用和自设预算无法证明订阅的剩余额度，因此不能作为额度回退。

所有 Provider 的菜单栏和侧栏汇总百分比只接受 `LIVE` 快照，取窗口中最低的剩余比例，避免遗漏周额度或模型特定限制。非实时快照的汇总值为 `—`，带来源标识的详情仍保留旧窗口。额度通知也只消费 `LIVE` 快照。
