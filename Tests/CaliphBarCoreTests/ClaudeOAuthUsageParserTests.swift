...
@Suite struct ClaudeOAuthUsageParserTests {
    @Test func parsesSessionWeeklyScopedAndPlan() throws {
        ...
        #expect(abs(snapshot.windows[0].usedFraction - 0.73) < 0.0001)
        #expect(abs(snapshot.windows[1].usedFraction - 0.07) < 0.0001)
        #expect(abs(snapshot.windows[2].usedFraction - 0.52) < 0.0001)
    }
}
